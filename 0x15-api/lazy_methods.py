...
"""Define useful functions for lazy developers (me)."""
...
def get_todos(user_id=None) -> "list[dict]":
    """
    Retrieve a list of todos for a given user if the user ID is provided.
...
def get_completed_tasks(tasks: "list[dict]") -> "list[dict]":
    """Return all the TODO tasks that the user has completed.
...
def export_all_to_json(users: "list[dict]", tasks: "list[dict]") -> None:
    ...
    """
    tasks_by_user = {}
    # Inserting a dummy element here so it's easier to reference user ids
